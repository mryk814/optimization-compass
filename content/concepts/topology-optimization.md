...
密度fieldから境界を取り出す後処理を加えても、元の連続境界を直接最適化したことにはなりません。
表現が違えば、更新できる設計と現れるmesh dependenceも変わります。
...
形状更新を含む場合は、geometry validity、mesh quality、state residualを別々に確認します。
...
[形状更新の失敗モード](#/learn/geometry-update-failure-modes)では、inversionや負のJacobian、checkerboard、mesh dependenceを同じ目的値にまとめない診断順を確認できます。
