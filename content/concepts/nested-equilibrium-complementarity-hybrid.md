...
入れ子の最適化、均衡条件、相補性、hybridのmode切替を、目的関数の種類ではなく、内側の解法・残差・離散状態として読み分けるための語彙です。
...
ここで、外側のobjectiveを1回評価しただけではありません。少なくともinner solveのiteration数、tolerance、終了status、inner objectiveまたはresidualを別に記録します。inner solveを途中で打ち切るtruncated solveや、反復を計算グラフへ展開するunrolled solveでは、得られた$y$が厳密な$y^*(x)$とは限りません。

hyperparameter selectionやinverse designも、parameterを変えるたびに学習・simulation・equilibrium solveを実行するなら、このnested構造として読めます。ただし、外側の問題をbilevelと呼ぶには、内側の解が外側の評価へどう定義されるかを明示します。
...
「solverの反復を微分した」のか、「十分に解いた後のsolution mapを微分した」のかも分けます。implicit differentiationは解写像の局所的な関係を使う場合がありますが、inner solveが未収束、解が非一意、constraint qualificationが崩れる場合には、その微分を無条件に保証できません。

## equilibriumは「最小化」と同じではない

equilibriumは、複数の主体や状態方程式が、他の条件を固定した一方的な改善だけでは動けない状態を表します。game equilibriumだけでなく、network、market、mechanicsの状態を均衡条件で定める場合もあります。
...
一方が正なら他方は0で、両方が0の境界も許します。KKT条件のactive constraintとmultiplier、contactのgapとnormal force、MPECのequilibrium constraintなどに現れます。
...
前者を解いたからといって、後者のmode選択まで最適化したことにはなりません。modeを離散変数で表すか、logical constraintで表すか、trajectoryのeventとして表すかを記録し、mode sequenceとswitching eventを目的値の履歴から推測しないようにします。chatteringやrelaxation artifactがある場合は、連続状態が滑らかでもmodeの意味が壊れていないか確認します。
...
[exact innerのTheater](#/visualization/SCENARIO_BILEVEL_REGRESSION_EXACT)は、固定した6回のouter updateを追います。
...
[finite relaxationのTheater](#/visualization/SCENARIO_BILEVEL_REGRESSION_RELAXED)は同じouter budgetとinner policyを使います。
...
[mode chatteringのTheater](#/visualization/SCENARIO_HYBRID_MODE_CHATTERING)は、既存contractで表せる最小のsecondary failure sliceです。
...
[制約class](#/learn/concept.constraint-class)で、equality・inequality・logical・complementarityを別々の可行性判定として確認します。trajectoryのstateと更新を分けるときは[trajectory変数](#/learn/concept.trajectory-variable)と[dynamics defect](#/learn/concept.dynamics-defect)へ進み、実際のsolver familyとの接続は[制約付きNLP](#/learn/family.constrained-nlp)と[optimal control](#/learn/family.optimal-control)で確認します。
