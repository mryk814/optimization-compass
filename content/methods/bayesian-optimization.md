...
教材は、次の順に見ると情報を一度に抱えずに済みます。

1. Theaterで、観測から次の評価点を選ぶ一巡を追う
2. [acquisition・noise・Random Searchの比較](#/compare/COMPARE_BO_ACQUISITION_NOISE_BASELINE)で、変えた条件を一つずつ読む
3. [同一costのmulti-fidelity比較](#/compare/COMPARE_BO_MULTIFIDELITY_COST)で、iteration数ではなく支払ったcostを揃える

個別のsensitivity runとbaselineは比較ページから開けます。
...
これはzero-mean、fixed kernel、noiselessに近い教育例です。
実務ではmean function、kernel hyperparameter、noiseを明示します。
numerical stabilityとacquisition optimizerも記録します。

## 理論・実装default・評価policy・推薦を分ける

Bayesian Optimizationの核は、観測からsurrogateを更新し、acquisitionで次の評価候補を決めることです。
特定libraryが選ぶdefault kernelやinput transformは、手法そのものではありません。
noise処理、acquisition optimizer、初期化点数も実装ごとに異なります。

BoTorchの公式資料には、既知のfidelity costを使うcost-awareな構成例があります。
ただし、libraryを選んだだけではcost modelとtarget fidelityは確定しません。
補正modelも別に設計します。
library versionとdefault値は実装記録として残し、普遍的な推奨値にはしません。

評価policyはさらに別です。
initial design、low/high fidelityのcost、parallel worker数をrun前に固定します。
batch / asynchronous、retry、failed・censored・timeoutの扱いも記録します。
停止条件を結果の確認後に変更しません。

methodの推薦優先度は、problemの変数型とbudgetから決めます。
noiseの性質も確認します。
失敗構造と比較証拠も必要です。
固定runのbest-so-farやlibrary defaultだけで推薦順位を上げません。
...
low fidelityはhigh fidelityの真値ではありません。
共通点を両fidelityで評価し、残差と候補順位を別々に記録します。
high fidelityで再確認した結果も分けて残します。

[low-fidelity biasのfailure Theater](#/theater/bayesian-optimization/SCENARIO_BO_1D_LOW_FIDELITY_BIAS)では、同じ2候補の順位がlow/highで反転します。
これは特定のbiasを持つ最小教材です。
実問題での発生頻度や補正modelの性能は示しません。

input / output scalingを変えたときにposteriorや提案が大きく崩れる場合は、kernel mismatchを疑います。
cross-validation residualが領域ごとに偏る場合も同様です。
uncertaintyが小さいのにhigh fidelity誤差が大きければ、model discrepancyを確認します。

failed領域の近傍を繰り返し提案する場合は、失敗を大きな目的値へ置換しません。
statusとfeasibility modelを監査します。
探索空間とretry policyも確認します。
