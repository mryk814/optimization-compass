...
real-time制御ではsolve time、warm start、fallback controllerもmodelの一部です。
...
最適化変数はcontrol列 $u_0,\ldots,u_{T-1}$です。候補controlで前進simulationし、trajectory costと制約を評価します。gradientを使う場合は、control変更が将来stateへ伝わる感度を計算します。

stateを独立変数にしないためdynamics equalityは自動的に満たされますが、unstable rolloutや長期感度が難しさになります。
...
- 初期時刻のgradientだけ巨大 → scalingやmultiple shootingを検討
