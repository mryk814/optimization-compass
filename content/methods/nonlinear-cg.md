...
$\beta_k$には複数の定義があります。実装上は、方向が降下方向でなくなった場合や一定間隔で、$p_k=-g_k$へ戻すrestartが重要です。更新式の名前だけでなく、line search条件とrestart policyを記録します。
...
- line search failureが続く → scaling、gradient、別globalizationを確認
- 進行は安定するが遅い → L-BFGSやtrust-regionと比較
- 初期値ごとに別解へ入る → multi-startやglobal-search familyを検討
...
線形共役勾配法と非線形共役勾配法は名前が近いものの、解いている対象と保証を同一視できません。非線形版ではline searchやrestartを含むalgorithm全体が挙動を決めます。
