...
まず[trajectory variable](#/learn/concept.trajectory-variable)でstateとcontrolを同時に持つ意味を確認し、[dynamics defect](#/learn/concept.dynamics-defect)でmesh上の等式制約を読みます。[path・terminal制約](#/learn/concept.path-terminal-constraints)で障害物・関節・入力の制約をどの時刻に評価するか、[時間discretization](#/learn/concept.time-discretization)でmesh refinementを確認します。観測ごとに再solveする用途では[receding horizon](#/learn/concept.receding-horizon)へ進み、solve timeとwarm startを別の運用条件として記録します。
