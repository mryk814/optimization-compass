...
球面上では勾配射影とretractionの動きを直接確認できます。
...
Stiefel多様体／Grassmann多様体／SO(3)などでは、接空間射影とretractionの実装が変わります。
...
[Riemannian updateのTheater](#/theater/learning/SCENARIO_SO3_RIEMANNIAN_ALIGNMENT)では、Lie algebra上のstepをexponential mapでSO(3)へ戻す流れを追えます。
目的関数値と、直交性・determinantの残差を分けて確認します。

[projected updateとのCompare](#/compare/COMPARE_SO3_PROJECTED_RIEMANNIAN)は、同じtarget・初期rotation・目的関数・step size・12回のoracle evaluationを使います。
変えるのは、接空間のstepを使うか、ambient stepをQR projectionするかだけです。
