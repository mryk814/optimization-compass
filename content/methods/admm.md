...
変数を分けることで、滑らかな項、非滑らかな正則化、制約、分散データを別々の部分問題として扱います。

- 見ているもの: primal residual、dual residual、objective value、subproblem solve error
- 動かしているもの: $x$、$z$、dual変数 $u$、penalty parameter $\rho$
- 前進の判断: primal residualとdual residualの両方が下がり、consensusが進むこと
...
です。
$f$ と $g$ を別々に扱うため、滑らかな項、非滑らかな正則化、制約、分散データなどを異なる部分問題へ分けられます。
...
分割後のsubproblemが元問題より難しくなる場合は、分割の利点を得にくくなります。
...
primal residualだけ小さくてもdual residualが大きければ、変数は制約を満たしつつまだ動いています。
逆も同様です。

::: warning
ADMMの1反復が軽いとは限りません。
各subproblemでfactorizationやinner optimizationを行う場合、inner solveの費用と精度も記録します。
:::
