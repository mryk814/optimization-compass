...
summary: 状態と段階に分解し、同じ部分問題の解を再利用してBellman再帰を解くstructure-firstな厳密法です。
source_ids: [S054]
prerequisites: []
...
状態と段階に分解し、同じ部分問題の解を再利用してBellman再帰を解くstructure-firstな厳密法です。

## 最適化問題を状態へ変える

DPでは、意思決定の履歴をすべて保持せず、将来に必要な情報だけをstateへまとめます。代表的な再帰は
