...
を扱います。$K$を非負orthant、second-order cone、positive semidefinite coneなどの組み合わせにすると、次のような一見異なる問題を同じ枠組みで表現できます。
...
各反復では、primal変数$x$、dual変数$y$、slack変数$s$を同時に更新するNewton方程式を、barrier parameterを下げながら解きます。[LP/QP barrier法](#/learn/barrier-lp-qp)と同じく中心pathに沿って進みますが、complementarity条件がcone $K$上のbarrier関数（対数障壁の一般化）に応じた形になる点が異なります。反復ごとにprimal feasibility residual、dual feasibility residual、duality gapが得られるため、停止判定や精度の確認がしやすいことが特徴です。
...
実務では、CVXPY（[S010](https://www.cvxpy.org/)）のようなmodeling層が、norm、固有値、robust制約などを人が読みやすい形式で受け取り、conic標準形へ変換（canonicalization）してからsolver層へ渡します。solver層にはClarabel、SCS、MOSEKなど複数の実装があり、
