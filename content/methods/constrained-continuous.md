...
制約付き問題では、objectiveが低いだけでは解を採用できません。
**feasibility、stationarity、active constraint、停止理由を分けて確認し、hard constraintを満たす候補の中でobjectiveを比較する**ことが出発点です。

- 見ているもの: objective、constraint residual、KKT residual、active constraint
- 動かしているもの: variableとconstraint処理のためのstep
- 前進の判断: feasible pointを保ったままobjectiveとstationarityが改善するか
- 別に確認するもの: derivative、scaling、初期点、solver status
...
| 項目 | 確認内容 |
|---|---|
| hard constraint | 何を必ず満たす必要があるか |
| tolerance | feasibilityとstationarityをどの閾値で判定するか |
| variable type | 連続変数だけか、discrete条件が混ざるか |
| derivative | objectiveとconstraintのgradient / Jacobianを得られるか |
| initial point | infeasible startやfeasible startの条件は何か |
| structure | sparsity、bounds、convexityを利用できるか |
...
infeasibleな点で目的値が低くても候補解ではありません。まず「何がhard constraintか」「どのtoleranceまで満たせば実行可能か」を定義します。
...
- KKT residualが停滞
- `success`でもviolationがtoleranceを超える
...
目的値、feasibility、stationarity、停止理由を一つのscoreへまとめず、それぞれの許容範囲とともに保存します。

::: warning
目的値が良いinfeasible点と、目的値はやや悪いfeasible点を同じrankingへ入れません。実行可能性、最適性、toleranceを別々に報告します。
:::
