...
[pendulum swing-up Case](#/gallery/EC027)では、dynamicsと初期・終端・path制約を固定します。予算とtoleranceも揃え、N=20とN=40のmeshを比較できます。primary Theaterはnode上のdefectと区間再構成のviolationを分けます。failure Theaterではgravityを変えたvalidation rolloutでmodel mismatchを確認します。

offlineの軌道を観測ごとに更新する場合は、[receding horizon](#/learn/concept.receding-horizon)を先に読みます。その後に[100Hz MPC Case](#/gallery/EC025)へ進みます。

接触やmode切替を含む問題では、[入れ子・均衡・相補性・hybrid構造](#/learn/concept.nested-equilibrium-complementarity-hybrid)を参照します。known contact scheduleとmode discoveryは別の問題として扱います。

安全制約をQPで補正するsafety filterは、[制約付き最適化](#/learn/family.constrained-nlp)と[LP・QP・錐最適化](#/learn/lp-qp-conic)へ接続します。state／control boundsを満たすことと、不変集合や安全性の保証を同一視しません。

## 次に読む

5つの概念を読んだら、[Direct Shooting](#/learn/direct-shooting)と[Direct Multiple Shooting](#/learn/multiple-shooting)のstateの扱いを比べます。path制約を置く[Direct Collocation](#/learn/direct-collocation)と、feedback則を含む[iLQR / DDP](#/learn/ilqr-ddp)も選び分けます。具体的な診断は[pendulum swing-up Case](#/gallery/EC027)と[mesh sensitivity Compare](#/compare/COMPARE_PENDULUM_COLLOCATION_MESH)で確認できます。
