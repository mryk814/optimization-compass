...
- JSON numberは有限なIEEE-754 binary64値として扱い、指数表記なしの正確な有限10進数へ展開します。`-0`は`0`、整数値のfloat（`2.0`や`1e20`を含む）は小数点なしで表します。payload／recordへ渡されたPythonの任意精度intも、有限binary64へ変換できる限り受理し、そのbinary64値をcanonical化します。
