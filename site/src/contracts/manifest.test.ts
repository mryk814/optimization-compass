...
describe("SiteManifest parser", () => {
  test("parses the versioned trace index asset exactly", () => {
    ...
    expect(parseSiteManifest(manifest).learning_journeys.path).toBe("learning-journeys.json");
    expect(parseSiteManifest(manifest).formulation_primer.path).toBe("formulation-primer.json");
