...
export interface VisualizationScenario {
  ...
  experiment: {
    oracle_policy: ("objective_value" | "gradient" | "constraint_value" | "constraint_jacobian" | "objective_vector" | "residual_vector" | "jacobian" | "state_solve" | "adjoint_solve")[];
...
function parseExperiment(raw: unknown, field: string): VisualizationScenario["experiment"] {
  ...
  return {
    oracle_policy: nonEmptyTextList(data.oracle_policy, `${field}.oracle_policy`).map((value) => {
      if (!["objective_value", "gradient", "constraint_value", "constraint_jacobian", "objective_vector", "residual_vector", "jacobian", "state_solve", "adjoint_solve"].includes(value)) throw new Error(`${field}.oracle_policy is invalid.`);
      return value;
