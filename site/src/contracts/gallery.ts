...
export interface GalleryIndex { contract_version: "1.0.0"; dataset_version: string; cases: GalleryCase[] }

export function parseGalleryIndex(raw: unknown): GalleryIndex {
  const data = object(raw, "gallery");
  if (data.contract_version !== "1.0.0") throw new Error("Unsupported gallery contract.");
  ...
  return { contract_version: "1.0.0", dataset_version: nonEmpty(data.dataset_version, "dataset_version"), cases };
}
