...
export function CoveragePage() {
  ...
  if (state.status === "error") return <p role="alert">接続状況を読み込めませんでした: {state.message}</p>;
...
function CoverageView({ report, journeys, subjectType, status, family, setSubjectType, setStatus, setFamily }: { report: CoverageReport; journeys: LearningJourneyIndex; subjectType: SubjectType | "all"; status: CoverageStatus | "all"; family: string; setSubjectType: (value: SubjectType | "all") => void; setStatus: (value: CoverageStatus | "all") => void; setFamily: (value: string) => void }) {
  ...
  return <section className="coverage-page">
    <header className="page-heading"><p className="eyebrow">接続状況</p><h1>Atlasの接続状況</h1><p>教材の深さを一律にせず、期待する学習成果と実在する成果物を分けて監査します。</p></header>
    <PageOrientation limits="接続状況は学習contractの接続状態を監査します。ページ数や接続率だけで、教材の質や手法の優劣を判断しません。" next={[{ label: "教材の進捗を見る", to: "/learn" }, { label: "Galleryのケースを見る", to: "/gallery" }, { label: "根拠資料を確認する", to: "/sources" }]} purpose="各method・problem・feature familyに、Map・診断・教材・可視化・根拠などがどこまで接続しているかを確認します。" readingSteps={["件数で全体の接続状態を見ます。", "優先スライスで次に補う学習成果を確認します。", "成果物一覧を絞り込み、欠けた接続と理由を追います。"]} />
    <p className="coverage-language-note"><strong>言語Coverage:</strong> 現在は日本語の説明を基準に監査します。英語の正式用語や用語aliasは検索metadataで、英語版の完備を意味しません。</p>
    ...
    <p className="coverage-baseline">リリース差分: 比較対象のbaseline未指定。初回スナップショットでは差分を推測しません。</p>
    <section aria-labelledby="priority-title"><h2 id="priority-title">優先スライス</h2><div className="coverage-priority-grid">{report.priorities.map((item) => <article key={item.slice_id}><p className="eyebrow">#{item.rank} · {item.total}/12</p><h3>{item.title_ja}</h3><p>{item.proposed_scope}</p><details><summary>優先理由</summary><ul>{Object.entries(item.factors).map(([name, factor]) => <li key={name}><strong>{name} {factor.score}/3:</strong> {factor.reason}</li>)}</ul></details></article>)}</div></section>
    <section aria-labelledby="inventory-title"><h2 id="inventory-title">成果物一覧</h2><div className="coverage-filters" aria-label="成果物一覧の絞り込み">
...
function JourneyCompleteness({ journeys }: { journeys: LearningJourneyIndex }) {
  ...
  return <section className="journey-coverage" aria-labelledby="journey-coverage-title">
    <div className="journey-coverage-heading">
      <div><p className="eyebrow">Case → Theater → Compare</p><h2 id="journey-coverage-title">学習経路の接続状況</h2></div>
    ...
    <div className="coverage-table-wrap" role="region" aria-label="学習経路の接続状況一覧" tabIndex={0}><table><thead><tr><th>学習経路</th><th>状態</th><th>不足している接続</th></tr></thead><tbody>
    ...
    <details className="journey-orphans">
      <summary><strong>未接続の成果物:</strong> 単独 {orphanCounts.standalone ?? 0}件 / 注意 {orphanCounts.warning ?? 0}件</summary>
