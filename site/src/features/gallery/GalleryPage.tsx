...
export function GalleryCasePage() {
  ...
  useEffect(() => { void loadGallery().then((index) => { const found = index.cases.find((candidate) => candidate.case_id === caseId); if (!found) throw new Error(`ケースID「${caseId}」は見つかりません。`); setItem(found); }, (caught: unknown) => setError(caught instanceof Error ? caught : new Error(String(caught)))); }, [caseId]);
