...
export function ContentIndexPage() {
  ...
  return <section className="atlas-page content-page"><header className="atlas-page-header"><p className="eyebrow">教材</p><h1>手法・概念を学ぶ</h1><p>直感、前提、1ステップ、可視化、コード、根拠をIDでつないだ教材です。</p></header><label className="content-search">教材を検索<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Nelder–Mead / convexity" /></label>{error && <p className="atlas-error" role="alert">{error.message}</p>}<div className="content-card-grid">{filtered.map((page) => <Link className="content-card" key={page.content_id} to={pageUrl(page)}><span>{page.kind === "method" ? "手法" : "概念"}</span><h2>{page.title_ja}</h2><p>{page.summary}</p><small>確認日 {page.last_reviewed}</small></Link>)}</div>{!error && pages.length > 0 && filtered.length === 0 && <p>一致する教材が見つかりません。</p>}</section>;
}
