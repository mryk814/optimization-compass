...
const MARKDOWN_PREVIEW_LABEL = "実装用プロンプトのMarkdownプレビュー";
...
test("Diagnoseから現在stateのMarkdownを編集・copyし、URLとscrollを維持する", async ({ page, context, baseURL }, testInfo) => {
  ...
  await expect(dialog.getByLabel(MARKDOWN_PREVIEW_LABEL)).toHaveValue(/Programming language: Python 3\.12/u);

  const preview = dialog.getByLabel(MARKDOWN_PREVIEW_LABEL);
...
test("Gallery caseは明記済みproblemだけをprefillし、環境をunknownで残す", async ({ page, baseURL }, testInfo) => {
  ...
  const dialog = page.getByRole("dialog", { name: "実装用プロンプトを作る" });
  await expect(dialog).toBeVisible();
  await expect(dialog.getByLabel("やりたいこと")).toHaveValue("1000件の配送先と時間窓を満たす良い配送ルートを、10分以内に得たい。");
  ...
  await expect(dialog.getByLabel(MARKDOWN_PREVIEW_LABEL)).toHaveValue(/Atlas origin: Gallery EC019/u);
  expect(page.url()).toBe(urlBefore);
