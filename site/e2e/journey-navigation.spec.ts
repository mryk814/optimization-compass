...
test("Case→Theater→Compare→Caseで共有可能なjourney文脈を保つ", async ({
  ...
  await expect(sharedPage.getByRole("complementary", { name: "ケースの学習経路" })).toContainText("Step 3/4");
...
test("文脈のないdeep linkは従来の安全なfallbackを使う", async ({ page, baseURL }) => {
  ...
  await expect(page.getByRole("complementary", { name: "ケースの学習経路" })).toHaveCount(0);
  await page.getByRole("button", { name: "← 動きの一覧に戻る" }).click();
...
test("dataset版が変わった共有URLは古いjourneyを誤復元しない", async ({ page, baseURL }) => {
  ...
  await expect(page.getByRole("alert")).toContainText("データ版が現在版と異なる");
  await expect(page.getByRole("complementary", { name: "ケースの学習経路" })).toHaveCount(0);
});
...
test("CompareからMethodへ進んでも元のCaseへ戻れる", async ({ page, baseURL }) => {
  ...
  await expect(page.getByRole("complementary", { name: "ケースの学習経路" })).toContainText("Step 4/4");
  await page.getByRole("button", { name: "← このCaseへ戻る" }).click();
  await expect(page).toHaveURL(/#\/gallery\/EC017\?state=/u);
});
