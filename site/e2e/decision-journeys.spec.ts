...
test("離散配分Caseを最適性証明、予算停止、CompareからCaseへ完走できる", async ({ page, baseURL }) => {
  ...
  const primaryTheater = page.getByRole("link", { name: /固定した1 runを追う/u });
  ...
  const alternate = page.getByRole("link", {
    name: "Alternate: 0-1 knapsack: node予算で停止",
  });
  ...
  const compare = page.getByRole("link", {
    name: "Compare: COMPARE_KNAPSACK_BNB_BUDGET",
  });
  ...
  await expect(metricValue(proof, "Incumbent")).toHaveText("15");
  ...
  await expect(metricValue(stopped, "Incumbent")).toHaveText("13");
