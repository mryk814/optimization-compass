...
def test_irrigation_case_is_a_bounded_cross_domain_teaching_case() -> None:
    gallery = json.loads(Path("data/seeds/site_gallery.json").read_text(encoding="utf-8"))
    case = next(
        item for item in gallery["cases"] if item["case_id"] == "irrigation-retrofit-selection"
    )

    assert case["domain"] == "agriculture"
    assert case["problem_archetype_id"] == "PA032"
    assert case["question_answers"]["Q01"] == "binary"
    assert case["map_node_id"] == "answer:Q01:binary"
    assert case["comparison_ids"] == ["COMPARE_IRRIGATION_BNB_BUDGET"]
    ...
    assert "教育用" in case["limitations"][0]
    assert "保証しない" in case["limitations"][1]
    assert "4品目knapsack" in case["practical_notes"]


def test_irrigation_comparison_changes_only_the_node_stop_limit() -> None:
    index = load_comparison_seed(Path("data/seeds/site_comparisons.json"), "test")
    comparison = next(
        item for item in index.comparisons if item.comparison_id == "COMPARE_IRRIGATION_BNB_BUDGET"
    )

    assert comparison.journey_id == comparison.case_id == "irrigation-retrofit-selection"
